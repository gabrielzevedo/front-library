...
const paddingSafe = plugin(function ({ addUtilities, e, config }) {
  // Paddings
  ...
  const paddingUtilities = paddingGenerators.flatMap((generator) => Object.values(paddings).flatMap(generator))
  ...
  const marginUtilities = marginGenerators.flatMap((generator) => Object.values(margins).flatMap(generator))
